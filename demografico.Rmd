...
pm6 <- read.spss('./data/Puntajes_Alumnos/PM6_TERCE.SAV', to.data.frame=T)
pm3 <- read.spss('./data/Puntajes_Alumnos/PM3_TERCE.SAV', to.data.frame=T)
...
pl6 <- read.spss('./data/Puntajes_Alumnos/PL6_TERCE.SAV', to.data.frame=T)
pl3 <- read.spss('./data/Puntajes_Alumnos/PL3_TERCE.SAV', to.data.frame=T)
...
a <- aggregate( puntaje_estandar_mat ~ country + gdp_2013 + nivel, m, mean) 
ggplot(a, aes(x=gdp_2013, y=puntaje_estandar_mat, fill=country, color=country, group=1)) + geom_point() + stat_smooth(method=lm, se=F) + geom_text(aes(label=country)) + facet_wrap( ~ nivel) + xlab("Ingreso per Capita") + ylab("Puntaje Promedio") + ggtitle("Puntaje promedio de matematica según ingreso per cápita por nivel (3ro y 6to)")
```

### Lectura
```{r puntaje_prom_gdp_lec, fig.width=12, fig.height=10, echo=F}
m <- merge(alumnos,gdp,by="country") 
a <- aggregate( puntaje_estandar_lec ~ country + gdp_2013 + nivel, m, mean) 
ggplot(a, aes(x=gdp_2013, y=puntaje_estandar_lec, fill=country, color=country, group=1)) + geom_point() + stat_smooth(method=lm, se=F) + geom_text(aes(label=country)) + facet_wrap( ~ nivel) + xlab("Ingreso per Capita") + ylab("Puntaje Promedio") + ggtitle("Puntaje promedio de matematica según ingreso per cápita por nivel (3ro y 6to)")
```
